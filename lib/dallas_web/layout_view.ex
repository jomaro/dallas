defmodule Dallas.LayoutView do
  ...
  defp asset_path(conn, asset) when asset in [:css, :js] do
    hash = Dallas.Assets.current_hash(asset)

    if function_exported?(conn.private.phoenix_router, :__dallas_prefix__, 0) do
      prefix = conn.private.phoenix_router.__dallas_prefix__()
